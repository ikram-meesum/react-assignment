...
const login = () => {
    ...
    axios
      ...
      .then(
        (response) => {
          ...
          navigate(`/comment/${response.data._id}`);
        },
        (error) => {
  ...
  return (
    <>
      ...
      {/* start alert */}
      <div
        className="flex items-center p-4 mt-6 text-sm text-blue-800 border border-blue-300 rounded-lg bg-blue-50 w-1/2 m-auto"
        role="alert"
      >
        <svg
      ...
      <section className="w-1/2 mx-auto shadow-2xl rounded-lg bg-slate-100">
        <h1 className="text-2xl text-center text-slate-700 pt-4 font-semibold mt-20 mb-10">
